...
describe("create new account",function(){
	...
	it('should create a student with userid newStudent1', async () => {
		const response = await registerStudent("newstudent1", "password");
		let student = await collections.Student.find({userid:"newstudent1"});
		assert.equal(student.length, 1);
	});

	it('should deny registration if userid is already taken', async () => {
		try{
			let response = await registerStudent("newstudent1", "password");
