...
const calculateCollegeScore = async (student) => {
  ...
  // calculate scores
  const maxGpaDiff = Math.max(...Object.values(gpaDiff));
  const maxSatmathDiff = Math.max(...Object.values(satmathDiff));
  const maxSatengDiff = Math.max(...Object.values(satengDiff));
  const maxActDiff = Math.max(...Object.values(actDiff));
  const maxCostDiff = Math.max(...Object.values(costDiff));

  for (let i = 0; i < allColleges.length; i++) {
    const college = allColleges[i];
    const students = studentsInCollege(similarStudents, college);
...
const lowerGPA = (collegeGPA) => 0.85 * collegeGPA;
const upperGPA = (collegeGPA) => 1.15 * collegeGPA;
